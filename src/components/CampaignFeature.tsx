  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl">
        <p className="mb-4 px-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-primary-600">
          {t(campaign.eyebrow)}
        </p>
        <div className="grid overflow-hidden rounded-2xl bg-[#11103f] text-white shadow-[0_16px_50px_rgba(15,12,70,0.12)] lg:grid-cols-[1.12fr_.88fr]">
          <div className="p-7 sm:p-9 lg:p-11">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-400/10 px-2.5 py-1 text-[9px] font-medium text-emerald-300">
            ...
            <h2
              ...
              className="mt-5 max-w-xl text-3xl font-semibold leading-[1.08] tracking-[-0.04em] sm:text-4xl"
            >
              {t(campaign.title)}
            </h2>
            ...
            <div className="mt-7 grid grid-cols-3 border-y border-white/10 py-4">
              {campaign.metrics.map((metric, index) => (
                <div
                  key={metric.label}
                  className={`${index ? "border-l border-white/10 pl-4 sm:pl-5" : ""}`}
                >
                  <strong className="block text-xl font-semibold tracking-[-0.03em] text-white">
                    {metric.value}
                  </strong>
            ...
            <Link
              href={campaign.href}
              className="mt-7 inline-flex h-9 items-center gap-2 rounded-full bg-white px-5 text-[11px] font-medium text-primary-900 transition hover:bg-primary-50"
            >
              {t("Zobrazit {count} akcí v kampani").replace(
                "{count}",
                String(campaign.activeEventCount),
              )}{" "}
              <ArrowIcon />
            </Link>
          </div>
              ...
              <div className="flex items-start justify-between gap-5">
                <div>
                  ...
                  <p className="mt-2 text-sm font-medium">
                    {t("Jednou uložit. Příště jen zaplatit.")}
                  </p>
                </div>
                <div className="shrink-0 text-right">
                  <div className="flex justify-end">
                    <span className="h-7 w-7 rounded-full bg-[#eb001b]" />
                    <span className="-ml-2.5 h-7 w-7 rounded-full bg-[#f79e1b] opacity-95" />
                  </div>
                  <span className="mt-1 block text-[8px] font-medium tracking-[0.04em] text-white/60">
                    mastercard
                  </span>
                </div>
              </div>
              <div className="mt-14 flex items-end justify-between border-t border-white/10 pt-5">
                <div>
                  <p className="text-[9px] text-white/40">
                    {t("Platba chráněná technologií Mastercard")}
                  </p>
                  <p className="mt-1 text-xs text-white/75">•••• 2048</p>
                </div>
                <span className="rounded-full bg-white/10 px-3 py-1 text-[9px] text-white/60">
                  {t("Připraveno k platbě")}
                </span>
              </div>
            </div>
          </div>
